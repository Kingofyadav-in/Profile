...
- Name: site.spec.js >> /pages/professional.html loads with correct title
- Location: tests/e2e/site.spec.js:60:3

# Error details

```
Error: page.goto: net::ERR_INTERNET_DISCONNECTED at https://kingofyadav.in/pages/professional.html
Call log:
  - navigating to "https://kingofyadav.in/pages/professional.html", waiting until "load"
...
> 61  |     const res = await page.goto(path);
  ...
  118 | // ── Static assets ──────────────────────────────────────────────────────────
  119 | 
  120 | test.describe("Static assets", () => {
  121 |   test("favicon loads", async ({ page }) => {
  122 |     await page.goto("/");
  123 |     const res = await page.request.get("/favicon/favicon.ico");
  124 |     expect(res.status()).toBeLessThan(400);
  125 |   });
  126 | 
  127 |   test("manifest.json is valid JSON", async ({ page }) => {
  128 |     const res  = await page.request.get("/manifest.json");
  129 |     expect(res.status()).toBe(200);
  130 |     const body = await res.json();
  131 |     expect(body).toHaveProperty("name");
  132 |   });
  133 | 
  134 |   test("robots.txt exists", async ({ page }) => {
  135 |     const res = await page.request.get("/robots.txt");
