...
;(function () {
  ...
  /* ── Detect theme early ── */
  let _theme = 'dark'
  try { _theme = localStorage.getItem('theme') || 'dark' } catch (_) {}
  const logoSrc = _theme === 'light' ? '/logo/day-logo.png' : '/logo/night-logo.png'
  ...
  const SOCIALS = [
    {
      ...
      external: false,
      svg: `<img class="footer-brand-logo" src="${logoSrc}" width="26" height="26" alt="" loading="lazy" />`,
    },
    {
      name: 'Facebook',
  ...
  const socialsHtml = SOCIALS.map(s => {
    ...
    const extAttrs = s.external === false ? '' : ' target="_blank" rel="noopener noreferrer"'
    const cls      = s.external === false ? ' footer-social-brand' : ''
    return `<a href="${s.url}"${extAttrs} aria-label="${s.name}" class="footer-social-link${cls}"${style}>${s.svg}</a>`
  }).join('')

  /* ── Micro SVG icons ── */
  const IC_PHONE    = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="13" height="13" aria-hidden="true"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>'
  const IC_MAIL     = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="13" height="13" aria-hidden="true"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>'
  const IC_CLOCK    = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="13" height="13" aria-hidden="true"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg>'
  const IC_LOCATION = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="13" height="13" aria-hidden="true"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>'
  ...
  const initActive = getISTHour() >= 10 && getISTHour() < 22
    ...
    <div class="footer-inner">
      ...
      <div class="footer-right">
        ...
        <div class="footer-info-row">
          ${IC_CLOCK}
          <span id="footerClock" aria-live="off" aria-atomic="true">--:--</span>
          <span class="footer-tz">IST</span>
        </div>
        <div class="footer-info-row footer-status-row" role="status" aria-live="polite" aria-atomic="true">
          <span class="footer-status-dot${initActive ? '' : ' offline'}" aria-hidden="true"></span>
          <span id="footerStatus">${initActive ? 'ACTIVE' : 'OFFLINE'}</span>
        </div>
      </div>
    </div>
  ...
  const dotEl    = footer.querySelector('.footer-status-dot')
  ...
  function syncStatus() {
    ...
    const active = getISTHour() >= 10 && getISTHour() < 22
    dotEl?.classList.toggle('offline', !active)
    if (statusEl) statusEl.textContent = active ? 'ACTIVE' : 'OFFLINE'
  }

  tick()
  const clockTimer  = setInterval(tick, 1000)
  const statusTimer = setInterval(syncStatus, 60_000)

  /* Cleanup on page hide to prevent leaks */
