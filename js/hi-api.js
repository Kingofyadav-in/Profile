...
/* ======================================================
   hi-api.js — Cloud Sync Layer (IndexedDB ↔ /api/hi/*)
   Patches hiPut/hiDelete to push every write to the API.
   Load AFTER hi-storage.js and call hiApiInit() on load.
...
function _idMap() {
  try { return JSON.parse(localStorage.getItem("hi_id_map") || "{}"); } catch { return {}; }
}

function hiApiSetId(localId, apiId, resource) {
  const m = _idMap();
  m[localId] = { apiId, resource };
  localStorage.setItem("hi_id_map", JSON.stringify(m));
}

function hiApiLookup(localId) { return _idMap()[localId] ?? null; }

function hiApiForgetId(localId) {
  const m = _idMap();
  delete m[localId];
  localStorage.setItem("hi_id_map", JSON.stringify(m));
}
...
async function hiApiFetch(path, method = "GET", body = null) {
  ...
  try {
    ...
    if (err.name === "AbortError") console.warn("[HI API] Timeout:", path);
    else console.warn("[HI API]", path, err.message);
    return null;
  } finally {
...
async function _hiApiPush(store, record) {
  if (!record?.id || !hiApiGetKey()) return;
...
async function hiApiPull() {
  ...
  try {
    ...
    console.warn("[HI API] Pull failed:", err.message);
  } finally {
    _hiApiPulling = false;
  }
}
...
(function patchHiStorage() {
  ...
  if (typeof _origDelete === "function") {
    window.hiDelete = async (store, id) => {
