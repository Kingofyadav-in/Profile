...
async function _hiApiPush(store, record) {
  ...
  if (store === 'tasks') {
    ...
    if (isNew) {
      ...
      hiApiFetch(`tasks/${apiId}`, 'PUT', body);
    }
    return;
  }
  ...
  if (store === 'personal') {
    if (record.type === 'goal') {
      ...
      else hiApiFetch(`goals/${apiId}`, 'PUT', body);
      return;
    }
    if (record.type === 'note') {
      ...
      else hiApiFetch(`notes/${apiId}`, 'PUT', body);
      return;
    }
  ...
  if (store === 'professional') {
    if (record.type === 'project') {
      ...
      else hiApiFetch(`projects/${apiId}`, 'PUT', body);
      return;
    }
    if (record.type === 'protask') {
      ...
      if (isNew) {
        ...
        hiApiFetch(`projects/${projEntry.apiId}`, 'PUT', { task_id: apiId, done: !!record.done });
      }
      return;
    }
  }
  ...
  if (store === 'social') {
    if (record.type === 'person') {
      ...
      else hiApiFetch(`contacts/${apiId}`, 'PUT', body);
      return;
    }
    if (record.type === 'event') {
      ...
      else hiApiFetch(`events/${apiId}`, 'PUT', body);
      return;
    }
  }
...
async function _hiApiDelete(localId) {
  ...
  if (!entry) return;
  await hiApiFetch(`${entry.resource}/${entry.apiId}`, 'DELETE');
...
async function hiApiPull() {
  ...
  try {
    ...
    const chat = await hiApiFetch(`chat/session-${today}`);
